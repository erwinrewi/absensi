...
def init_db():
    ...
    # Only store exceptions from default:
    # If no row -> default = "MASUK"
    # If row exists -> override = "LIBUR"
    cur.execute("""
...
def clear_override(employee_ids, work_date: date):
    # Clearing override means back to default MASUK
    for eid in employee_ids:
        execute("""
...
def build_report(start_date: date, end_date: date):
    """
    Return per-employee summary:
    - total_days (inclusive)
    - libur_days (count overrides)
    - masuk_days = total_days - libur_days (default)
    """
    employees = get_active_employees()
    if employees.empty:
        return pd.DataFrame()

    # Count LIBUR overrides per employee in range
    overrides = query_df("""
...
with tab1:
    ...
    with colA:
        ...
        full_name = st.text_input("Nama lengkap", placeholder="Contoh: Andi Pratama")
        employee_code = st.text_input("Kode karyawan (opsional, unik)", placeholder="Contoh: EMP-001")
    ...
    with colB:
        ...
        if df_emp.empty:
            ...
            selected_id = st.selectbox("Pilih karyawan", options=ids, format_func=lambda x: df_emp.loc[df_emp["id"] == x, "full_name"].values[0])
            new_state = st.radio("Status", options=["Aktif", "Nonaktif"], horizontal=True)
...
with tab2:
    ...
    if df_active.empty:
        ...
        with col1:
            ...
            mode = st.radio("Untuk siapa?", ["Satu karyawan", "Semua karyawan aktif"], horizontal=True)
            notes = st.text_input("Catatan (opsional)", placeholder="Contoh: Cuti bersama / Izin pribadi")
            ...
            if st.button("Tandai LIBUR", type="primary"):
...
with tab3:
    ...
    if df_active.empty:
        ...
        with colR1:
            ...
            y = st.number_input("Tahun", min_value=2000, max_value=2100, value=today.year, step=1)
            m = st.number_input("Bulan", min_value=1, max_value=12, value=today.month, step=1)
            ...
            st.download_button("Download CSV Bulanan", data=csv_m, file_name=f"rekap_{y}-{int(m):02d}.csv", mime="text/csv")

        with colR2:
            st.markdown("### Rekap tahunan")
            y2 = st.number_input("Tahun (tahunan)", min_value=2000, max_value=2100, value=today.year, step=1, key="year2")
            ...
            st.download_button("Download CSV Tahunan", data=csv_y, file_name=f"rekap_{y2}.csv", mime="text/csv")

        with colR3:
            st.markdown("### Catatan logika (supaya kamu tidak salah paham)")
            st.write(
                "- Sistem ini **tidak menghitung jam**.\n"
                "- Data yang disimpan hanya **pengecualian LIBUR**.\n"
                "- Maka: **MASUK = default** untuk semua tanggal di rentang rekap, kecuali ada override LIBUR.\n"
                "- Konsekuensi: kalau ada karyawan yang benar-benar alfa/tidak masuk, sistem ini tidak bisa bedakan, "
                "karena kamu tidak minta status itu.\n"
            )
