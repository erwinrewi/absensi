...
# Unique key helper
# -----------------------
APP_KEY_PREFIX = st.session_state.get("_app_key_prefix")
if APP_KEY_PREFIX is None:
    APP_KEY_PREFIX = uuid.uuid4().hex[:8]
    st.session_state["_app_key_prefix"] = APP_KEY_PREFIX

def k(name: str) -> str:
    return f"{APP_KEY_PREFIX}_{name}"
...
with tab1:
    ...
    with colA:
        ...
        full_name = st.text_input("Nama lengkap", placeholder="Contoh: Andi Pratama", key=k("emp_full_name"))
        employee_code = st.text_input("Kode karyawan (opsional, unik)", placeholder="Contoh: EMP-001", key=k("emp_code"))

        if st.button("Tambah", type="primary", key=k("btn_add_emp")):
            if not full_name.strip():
                st.error("Nama tidak boleh kosong.")
            else:
                try:
                    execute(
                        "INSERT INTO employees(full_name, employee_code, is_active, created_at) VALUES(?, ?, 1, ?)",
    ...
    with colB:
        ...
        if df_emp.empty:
            ...
            selected_id = st.selectbox(
                ...
                key=k("emp_select_status")
            )
            new_state = st.radio("Status", options=["Aktif", "Nonaktif"], horizontal=True, key=k("emp_radio_status"))
            if st.button("Simpan status", key=k("btn_save_status")):
...
with tab2:
    ...
    if df_active.empty:
        ...
        with col1:
            ...
            d = st.date_input("Tanggal", value=date.today(), key=k("libur_date"))
            mode = st.radio("Untuk siapa?", ["Satu karyawan", "Semua karyawan aktif"], horizontal=True, key=k("libur_mode"))
            notes = st.text_input("Catatan (opsional)", placeholder="Contoh: Cuti bersama / Izin pribadi", key=k("libur_notes"))
            ...
            if st.button("Tandai LIBUR", type="primary", key=k("btn_set_libur")):
                set_libur(target_ids, d, notes=notes.strip())
                st.success("LIBUR tersimpan.")

        with col2:
            st.markdown("### Batalkan LIBUR (kembali default MASUK)")
            d2 = st.date_input("Tanggal yang dibatalkan", value=date.today(), key=k("cancel_date"))
            mode2 = st.radio("Untuk siapa dibatalkan?", ["Satu karyawan", "Semua karyawan aktif"], horizontal=True, key=k("cancel_mode"))
            ...
            if mode2 == "Satu karyawan":
                emp_id2 = st.selectbox(
                    ...
                    options=df_active["id"].tolist(),
                    format_func=lambda x: df_active.loc[df_active["id"] == x, "full_name"].values[0],
                    key=k("cancel_emp_one")
                )
                target_ids2 = [emp_id2]
            else:
                target_ids2 = df_active["id"].tolist()

            if st.button("Batalkan LIBUR", key=k("btn_cancel_libur")):
                clear_override(target_ids2, d2)
                st.success("Override LIBUR dihapus.")

        st.markdown("### Daftar override LIBUR")
        df_over = query_df("""
...
with tab3:
    ...
    if df_active.empty:
        ...
        today = date.today()
        colR1, colR2 = st.columns([1, 1], gap="large")

        with colR1:
            st.markdown("### Rekap bulanan")
            y = st.number_input("Tahun", min_value=2000, max_value=2100, value=today.year, step=1, key=k("rep_month_year"))
            m = st.number_input("Bulan", min_value=1, max_value=12, value=today.month, step=1, key=k("rep_month_month"))
            ...
            st.download_button(
                ...
                data=rep_m.to_csv(index=False).encode("utf-8"),
                file_name=f"rekap_{y}-{int(m):02d}.csv",
                mime="text/csv",
                key=k("dl_month")
            )
        ...
        with colR2:
            ...
            y2 = st.number_input("Tahun (tahunan)", min_value=2000, max_value=2100, value=today.year, step=1, key=k("rep_year_year"))
            ...
            st.download_button(
                ...
                data=rep_y.to_csv(index=False).encode("utf-8"),
                file_name=f"rekap_{y2}.csv",
                mime="text/csv",
                key=k("dl_year")
            )
